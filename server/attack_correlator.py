...
class AttackCorrelator:
    ...
    def correlate(self, scan_result: ScanResult) -> list[VulnerabilityMatch]:
        ...
        # Убираем дубликаты (по CVE ID + attack vector ID)
        self.results = self._deduplicate(self.results)

        print(f"[+] Корреляция завершена. Результатов: {len(self.results)}")
        return self.results
    ...
    def _evaluate_feasibility(self, cve: dict, av: AttackVector) -> tuple:
        ...
        if required_ports and not any(p in open_port_nums for p in required_ports):
            return (
                AttackFeasibility.NOT_FEASIBLE,
                f"Необходимые порты ({', '.join(map(str, required_ports))}) закрыты. "
                f"Атака не может быть проведена."
            )
    ...
    def _deduplicate(self, results: list[VulnerabilityMatch]) -> list[VulnerabilityMatch]:
        ...
        unique = []
        for r in results:
            key = (r.cve_id, r.attack_vector_id)
            if key not in seen:
                seen.add(key)
                unique.append(r)
        return unique
    ...
    def get_summary(self) -> dict:
        ...
        critical = sum(1 for r in self.results if r.severity == "CRITICAL" and r.feasibility != AttackFeasibility.NOT_FEASIBLE.value)
        high = sum(1 for r in self.results if r.severity == "HIGH" and r.feasibility != AttackFeasibility.NOT_FEASIBLE.value)
